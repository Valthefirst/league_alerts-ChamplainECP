...
const API_BASE_URL =
  "https://dolphin-app-sxvxi.ondigitalocean.app/api/v1/subscriptions";
...
export const subscribeToNotifications = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(
        error.response?.data || "An error occurred while subscribing.",
      );
    } else {
...
export const unsubscribeFromNotifications = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(
        error.response?.data || "An error occurred while unsubscribing.",
      );
    } else {
      throw new Error("An unknown error occurred.");
    }
