...
//    private CloudinaryService cloudinaryService;
    ...
    void whenValidArticleTag_thenReturnAllArticle() {
        ...
        Categories categories = Categories.builder().categoryName("NFL").build();
...
//    }
    ...
    void searchArticles_ShouldReturnMatchingArticles() {
        ...
        Categories tag = Categories.builder()
                .categoryName("NBA")
                .build();

        Categories tag2 = Categories.builder()
                .categoryName("NFL")
                .build();
        ...
        // Mock the repository to return a Flux<Article>
        when(articleRepository.findByCategoryContainingAndTitleContainingIgnoreCase(tag.getCategoryName(), query))
                .thenReturn(Flux.just(article1, article2));

        // Act and Assert using StepVerifier
        StepVerifier.create(articleService.searchArticles(tag.getCategoryName(), query))
        ...
        verify(articleRepository).findByCategoryContainingAndTitleContainingIgnoreCase(tag.getCategoryName(), query);
